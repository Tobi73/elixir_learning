defmodule Vocial.Votes do
  ...
  def already_voted?(poll_id, ip_address) do
      votes = (from vr in VoteRecord, where: vr.poll_id == ^poll_id 
                and vr.ip_address == ^ip_address)
      |> Repo.aggregate(:count, :id)
      votes > 0
  end

  defp update_option(option, attrs) do
    option
    |> Option.changeset(attrs)
    |> Repo.update()
  end
  ...
  defp upload_file(_, _) do
    {:ok, nil}
  end

  defp save_upload(_poll, _image_data, nil) do
    {:ok, nil}
  end
