defmodule Vocial.VotesTest do
  ...
  describe "polls" do
    ...
    def poll_fixture(attrs \\ %{}) do
      with create_attrs <- Enum.into(attrs, @valid_attrs),
           ...
           poll <- Repo.preload(poll, [:options, :vote_records, :image]) do
        poll
      end
    end
    ...
    test "create_poll_with_options/2 does not create poll or options with bad data", %{user: user} do
      title = "Bad poll"
  ...
  describe "options" do
    test "create_option/1 creates an option on poll", %{user: user} do
    ...
    test "vote_on_option/1 adds a vote to a particular option", %{user: user} do
      with {:ok, poll} = Votes.create_poll(%{title: "Sample poll", user_id: user.id}),
        ...
        votes_updated = updated_option.votes
        assert votes_before + 1 == votes_updated
      end
    end
  end
end
